...
## here i aggreagated data to the point, maxium extnet possible while still keeping all the functions in tact
...
def update_rs(d, dt):
     for body in d.keys():
        ...
        r[1] += dt * vy
        r[2] += dt * vz
def compute_deltas_and_bs(d,dt):
    seenit = []
    for body1 in d.keys():
        for body2 in d.keys():
            if (body1 != body2) and not (body2 in seenit):
                ([x1, y1, z1], v1, m1) = d[body1]
                ([x2, y2, z2], v2, m2) = d[body2]
...
def report_energy(e=0.0):
    ...
    '''
    e=compute_deltas_and_energy(e=e,d=BODIES)
    for body in BODIES.keys():
        (r, [vx, vy, vz], m) = BODIES[body]
        e += m * (vx * vx + vy * vy + vz * vz) / 2.
    return e

def report_advance(loops, iterations,dt):
...
def nbody(loops, reference, iterations):
    ...
    report_advance(loops, iterations,0.01)
